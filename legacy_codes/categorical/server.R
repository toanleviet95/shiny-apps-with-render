...
shinyServer(function(input, output, session){
  ...
  output$table_data <- DT::renderDataTable({
    ...
    else datatable(dataInput(), rownames = TRUE )
  })
  ## Goodness-of-fit tab
  ...
  output$p_0 <- renderUI({
    ...
    lapply(seq(z),function(i){
      numericInput(inputId = paste0("range",paste0(i)), label = paste0(i,"-th expected proportion"), value = 1/z, 
                   min = 0, max = 1)  
    })
  })
  ...
  output$table_expection_1 <- renderTable({
    ...
    tab_expect_1
  }, digits = 4, bordered = FALSE, align = "r", striped = TRUE, width = '100%',
