...
shinyServer(function(input, output, session){
  ...
  observe({
    if(is.null(input$butt_test_2)) return(NULL)
    if(input$butt_test_2 == 0) return(NULL)
    isolate({ 
      updateTabsetPanel(session, "out1", selected = "Two-sample testing")
    })
  })
  ...
  output$hypothesis <- renderUI({
    if(is.null(dataInput())) return(NULL)
    if(is.null(input$type_jobs)) return(NULL)
    if(is.null(input$selected_mean_0)) return(NULL)
    # input$butt_select
    isolate({
      radioButtons(inputId = "null_hypo", label = "The null hypothesis",
                   choiceNames = list(paste("\u03BC =", input$selected_mean_0), 
  ...
  output$res_test_1 <- renderTable({
    # if(is.null(input$butt_select)) return(NULL)
    if(is.null(input$butt_test_1)) return(NULL)
    if(input$butt_test_1 == 0) return(NULL)
    input$butt_test_1
    isolate({
      res_test <<- t.test(x = dataInput()[,input$selected_var_test_1], alternative = input$null_hypo,
      ...
      res_tab <- c(round(res_test$estimate, 4), 
    ...
    as.data.frame(tab_1)
  }, digits = 4, bordered = FALSE, align = "lr", striped = TRUE, rownames = TRUE)
  ##
  ...
  output$hypothesis_2 <- renderUI({
    ...
    # input$butt_select
    isolate({
      radioButtons(inputId = "null_hypo_2", label = "The null hypothesis",
                   choiceNames = list(HTML("<p>\U03BC<sub>1</sub> = \U03BC<sub>2</sub></p>"), 
  ...
  output$res_test_2 <- renderTable({
    ...
    input$butt_test_2
    isolate({
      res_test_2 <<- t.test(x = dataInput()[,input$selected_var_test_2_1],
      ...
      if(isTRUE(input$indepe)){
        res_tab_2 <- c(round(res_test_2$estimate[1], 4), round(res_test_2$estimate[2], 4),
