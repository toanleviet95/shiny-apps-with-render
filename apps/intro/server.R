server <- function(input, output, session) {
  ...
  observe({
    if (is.null(input$butt_sum_data)) {
      return(NULL)
    }
    if (input$butt_sum_data == 0) {
      return(NULL)
    }
    isolate({
  ...
  observe({
    if (is.null(input$butt_run_plot)) {
      return(NULL)
    }
    if (input$butt_run_plot == 0) {
      return(NULL)
    }
    isolate({
  ...
  output$summary <- renderTable(
    {
      ...
      isolate({
        if (input$type_vars == "numb") {
          u <- c(
            as.numeric(summary(dataInput()[, input$selected_var])[1:6]),
          ...
          tab <- cbind(u)
          rownames(tab) <- c(
            "Minimum", "Q1", "Median-Q2", "Mean", "Q3", "Maximum", "Variance",
  ...
  plotInput <- function() {
    if (input$type_vars == "numb") {
      if (input$type_plot == "boxpl") {
        boxplot(dataInput()[, input$selected_var],
          main = paste("Box plot of", input$selected_var),
          col = "forestgreen", pch = 16
        )
      }
      if (input$type_plot == "hist") {
        hist(dataInput()[, input$selected_var],
      ...
      if (input$type_plot == "bar_plot") {
        xx <- barplot(res_tab[-length(res_tab)],
          ylab = "Frequency",
          main = paste("Bar chart of", input$selected_var)
        )
        text(
          x = xx, y = as.numeric(res_tab[-length(res_tab)]),
