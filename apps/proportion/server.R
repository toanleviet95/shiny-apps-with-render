server <- function(input, output, session) {
  ...
  observe({
    if (is.null(input$butt_test)) {
      return(NULL)
    }
    if (input$butt_test == 0) {
      return(NULL)
    }
    input$butt_test
    isolate({
  ...
  output$res_test <- renderTable(
    {
      ...
      isolate({
        ...
        res_test <<- z.test(
          n = input$num_trails, p0 = input$selected_p_0, phat = input$p_hat,
        ...
        res_tab <- c(
          round(res_test$estimate, 4), round(input$num_trails * input$selected_p_0, 4),
