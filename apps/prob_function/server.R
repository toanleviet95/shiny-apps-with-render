...
server <- function(input, output) {
  output$model_prob <- renderUI({
    if (input$compute) {
      radioButtons(
        inputId = "model", label = "Model of probability",
        choiceNames = list(
  ...
  plotInput <- function() {
    if (!input$compute) {
      ...
      barplot(dbinom(y, input$num_obs, input$prob),
        ylab = "Pr(X = x)", xlab = "x", space = 0.5,
        col = col_bar
      )
    }
  }
  ...
  output$summary <- renderTable(
    {
      ...
      isolate({
        u <- c(
          input$num_obs * input$prob, input$num_obs * input$prob * (1 - input$prob),
